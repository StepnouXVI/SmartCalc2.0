...
shared_ptr<OperationsData> OperationSet::getData() { return _data; }

void OperationSet::fillBinaryOperations() {
  std::pair<const char *, std::function<double(double, double)>>
      binaryOperations[] = {
  ...
  for (const auto &operation : binaryOperations) {
    addBinaryOperation(operation.first, operation.second, Left,
                       operation.first == "^" ? High : Medium);
  }
}
...
void OperationSet::addBinaryOperation(const char *lexeme,
                                      function<double(double, double)> op,
                                      Associativity associativity,
                                      Priority priority) {
  (*_data->Operations)[lexeme] =
      make_shared<BinaryOperation>(op, associativity, priority);
  if (strlen(lexeme) == 1) {
    _data->Operators->insert(lexeme[0]);
  }
}

void OperationSet::addUnaryOperation(const char *lexeme,
                                     function<double(double)> op,
                                     Associativity associativity,
                                     Priority priority) {
  (*_data->Operations)[string(lexeme)] =
      make_shared<UnaryOperation>(op, priority, associativity);
  if (strlen(lexeme)) {
    _data->Operators->insert(lexeme[0]);
  }
}
