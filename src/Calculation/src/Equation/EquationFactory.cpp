#include <Calculation/Equation/EquationFactory>
...
shared_ptr<Equation> EquationFactory::CreateEquation(shared_ptr<string> expression)
{
...
shared_ptr<EquationWithX> EquationFactory::CreateEquationWithX(shared_ptr<string> expression)
{
    auto lexemes = _parser->Preprocess(expression);
    _resultList = make_shared<std::list<OperationPtr>>();
...
EquationFactory::OperationType EquationFactory::getOperationType(string &current)
{
    if (current == "(")
        return OpenBracket;
    if (current == ")")
...
{
    while (!_tmpStack->empty() && _tmpStack->top()->GetPriority() != Priority::OpenBracket)
    {
        _resultList->push_front(_tmpStack->top());
        _tmpStack->pop();
    }
...
void EquationFactory::xProcession()
{
    _resultList->push_front(_x);
}
} // namespace Equations

void Equations::EquationFactory::leftAssociativeProcession(shared_ptr<IOperation> operation)
{
