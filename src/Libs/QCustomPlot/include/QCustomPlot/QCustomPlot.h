...
enum Interaction
{
    ...
    ,
    iSelectPlottablesBeyondAxisRect = 0x100 ///< <tt>0x100</tt> When performing plottable selection/hit tests,
...
class QCPLabelPainterPrivate
{
  ...
  protected:
    ...
    virtual QByteArray generateLabelParameterHash() const; // TODO: get rid of this in favor of invalidation flag upon
                                                           // setters?
...
template <class DataType> void QCPDataContainer<DataType>::add(const QVector<DataType> &data, bool alreadySorted)
{
    ...
    if (alreadySorted && oldSize > 0 &&
        !qcpLessThanSortKey<DataType>(*constBegin(),
                                      *(data.constEnd() - 1))) // prepend if new data is sorted and keys are all smaller
...
{
    if (isEmpty() || !qcpLessThanSortKey<DataType>(data, *(constEnd() - 1))) // quickly handle appends if new data key
...
template <class DataType> void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    mPreallocSize += int(itEnd - it); // don't actually delete, just add it to the preallocated block
...
template <class DataType> void QCPDataContainer<DataType>::preallocateGrow(int minimumPreallocSize)
{
    ...
    newPreallocSize += (1u << qBound(4, mPreallocIteration + 4, 15)) - 12; // do 4 up to 32768-12 preallocation,
                                                                           // doubling in each intermediate iteration
    ++mPreallocIteration;
...
class QCP_LIB_DECL QCustomPlot : public QWidget
{
  ...
  public:
    ...
    */
    ...
    template <class ItemType> ItemType *itemAt(const QPointF &pos, bool onlySelectable = false) const;
...
PlottableType *QCustomPlot::plottableAt(const QPointF &pos, bool onlySelectable, int *dataIndex) const
{
    ...
    double resultDistance = mSelectionTolerance; // only regard clicks with distances smaller than
    ...
    foreach (QCPAbstractPlottable *plottable, mPlottables)
    {
        ...
        if (!currentPlottable ||
            (onlySelectable && !currentPlottable->selectable())) // we could have also passed onlySelectable to
                                                                 // the selectTest function, but checking here is
                                                                 // faster, because we have access to
                                                                 // QCPAbstractPlottable::selectable
            continue;
        if (currentPlottable->clipRect().contains(pos.toPoint())) // only consider clicks where the plottable is
                                                                  // actually visible
        {
            QVariant details;
...
*/
template <class ItemType> ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const
{
    ...
    foreach (QCPAbstractItem *item, mItems)
    {
        ...
        if (!currentItem || (onlySelectable && !currentItem->selectable())) // we could have also passed onlySelectable
        ...
        if (!currentItem->clipToAxisRect() ||
            currentItem->clipRect().contains(pos.toPoint())) // only consider clicks inside axis cliprect of the
...
class QCP_LIB_DECL QCPPolarGraph : public QCPLayerable
{
  ...
  public:
    ...
    */
    ...
    // introduced virtual methods:
    virtual double selectTest(const QPointF &pos, bool onlySelectable,
                              QVariant *details = 0) const; // actually introduced in QCPLayerable as non-pure, but we
