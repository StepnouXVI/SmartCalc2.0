...
std::shared_ptr<std::queue<std::string>> LexemeParser::Preprocess(
...
LexemeParser::SymbolType Preprocessors::LexemeParser::getTypeOfSymbol(
    char symbol) {
  ...
  if (symbol == '(' || symbol == ')') return bracket;

  if (_operationData->Operators->find(symbol) !=
      _operationData->Operators->end())
    return operand;
...
void Preprocessors::LexemeParser::lettersProcession() {
  ...
  while (isLetter(*_currentSymbol)) _currentSymbol++;
...
void Preprocessors::LexemeParser::digitProcession() {
  ...
  if (dotAmount > 1) throw std::runtime_error("Invalid digit.");
