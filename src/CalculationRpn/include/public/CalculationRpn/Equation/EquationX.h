...
class EquationWithX : private Equation {
 ...
 public:
  ...
  void Execute(double start, double end, size_t numberOfSteps,
               std::vector<double> &result);

  double operator()(double x);
  std::vector<double> operator()(double start, double end,
                                 size_t numberOfSteps);
  void operator()(double start, double end, size_t numberOfSteps,
                  std::vector<double> &result);
};
}  // namespace Equations
