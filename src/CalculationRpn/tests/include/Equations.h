...
class EquationTests : public ::testing::Test {
 protected:
  void SetUp() override {
    auto add =
        make_shared<BinaryOperation>([](double a, double b) { return a + b; },
                                     Associativity::Left, Priority::Low);
    auto negate = make_shared<UnaryOperation>(
        [](double a) { return -a; }, Priority::Low, Associativity::Left);

    auto n1 = make_shared<NumberOperation>(1);
    auto n2 = make_shared<NumberOperation>(2);

    correctData->push_front(add);
    correctData->push_front(negate);
    correctData->push_front(n1);
    correctData->push_front(n2);

    correctDataWithX->push_front(add);
    correctDataWithX->push_front(negate);
    correctDataWithX->push_front(x);
    correctDataWithX->push_front(n2);

    incorrectData->push_front(n2);
    incorrectData->push_front(n1);

    incorrectSizeData->push_front(add);
  }
...
shared_ptr<list<shared_ptr<Operations::IOperation>>>
    EquationTests::correctDataWithX =
        make_shared<list<shared_ptr<Operations::IOperation>>>();
shared_ptr<list<shared_ptr<Operations::IOperation>>>
    EquationTests::correctData =
        make_shared<list<shared_ptr<Operations::IOperation>>>();
double EquationTests::correctDataResult = 1;
shared_ptr<list<shared_ptr<Operations::IOperation>>>
    EquationTests::incorrectSizeData =
        make_shared<list<shared_ptr<Operations::IOperation>>>();
shared_ptr<list<shared_ptr<Operations::IOperation>>> EquationTests::emptyData =
    make_shared<list<shared_ptr<Operations::IOperation>>>();
shared_ptr<list<shared_ptr<Operations::IOperation>>>
    EquationTests::incorrectData =
        make_shared<list<shared_ptr<Operations::IOperation>>>();
